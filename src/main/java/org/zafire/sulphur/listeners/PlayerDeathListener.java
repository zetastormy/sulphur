...
public class PlayerDeathListener implements Listener {
    ...
    public PlayerDeathListener(final MessageUtils messageUtils, final EconomyUtils economyUtils, final Economy economy, final CacheHandler cacheHandler) {
    ...
    public void onPlayerDeath(final PlayerDeathEvent event) {
        ...
        if (player.getKiller() != null && player.getKiller() != player && player.getKiller() instanceof Player) {
            ...
            if (player.hasPermission("sulphur.deathtax.bypass") && totalLostPvp != 0.0) {
                messageUtils.sendMessage(player, false, "&2&lSurvival &8|| &7Te encuentras exento del impuesto por muerte.");
                messageUtils.sendMessage(player.getKiller(), false, "&2&lSurvival &8|| &c" + player.getDisplayName() + " &7se encuentra exento del impuesto por muerte");
                return;
            }

            if (economy.getBalance(player) <= 0.0) {
                messageUtils.sendMessage(player, false, "&4&lCombate &8|| &7No se te ha cobrado el impuesto por muerte, ya que no tienes suficiente dinero.");
                return;
            }

            if (totalLostPvp != 0.0) {
                messageUtils.sendMessage(player, false, "&4&lCombate &8|| &7Se te ha cobrado &a" + moneyFormatted + " &7por morir.");
                economy.withdrawPlayer(player, totalLostPvp);
            }
        }

        if (player.getKiller() == event.getEntity() || !(player.getKiller() instanceof Player) || player.getKiller() == null) {
            ...
            if (economy.getBalance(player) <= 0.0)  {
                messageUtils.sendMessage(player, false, "&2&lSurvival &8|| &7No se te ha cobrado el impuesto por muerte, ya que no tienes suficiente dinero.");
                return;
            }

            if (totalLostPve != 0.0) {
                messageUtils.sendMessage(player, false, "&2&lSurvival &8|| &7Se te ha cobrado &a" + moneyFormatted + " &7por morir.");
